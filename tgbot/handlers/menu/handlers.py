...
from tgbot.static_text import desciption_of_OLM
...
def Give_test(update: Update, context: CallbackContext) -> None:
    ...
    if query.data=="get_back":
        ...
        selected_images = list(TestFile.objects.all())
...
def files_handler(update, context):
    ...
    if u.is_admin:
        if message.caption:
            caption = message.caption
            vv = TestFile(file_id=file_id,description=caption)
            vv.save()   
            update.message.reply_text(text="Saved !")
...
def Name_Handler(update, context):
    full_name = update.message.text.strip()  # Remove leading/trailing spaces
    ...
    if len(name_parts) == 2:
        ...
        # Save the first and last names in user_data
...
def Phone_number_handler(update, context):
    ...
    # Check if the message only contains digits and has the desired length
